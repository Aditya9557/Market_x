...
// ─── HERO APPLICATION SCHEMA ────────────────────────────────

export const heroApplicationSchema = z.object({
    fullName: z.string().min(2).max(200).trim(),
    campusEmail: z.string().email().max(255).trim().toLowerCase(),
    phone: z.string().regex(/^\d{10}$/, 'Phone must be exactly 10 digits'),
    zone: z.string().min(1).trim(),
    preferredHours: z.array(z.enum(['morning', 'afternoon', 'night'])).min(1),
